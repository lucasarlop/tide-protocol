...
def _load_config(root: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    try:
        ...
        return ({'strategy': 'balanced', 'allow_xhigh': True}, [f'ignored invalid {path.relative_to(root)}: {exc}'])
    if not isinstance(parsed, dict):
        return ({'strategy': 'balanced', 'allow_xhigh': True}, [f'ignored invalid {path.relative_to(root)}: expected JSON object'])
...
def _blocking_findings(runtime: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [item for item in review.get('findings', []) if isinstance(item, dict) and item.get('severity') == 'blocking']

def _has_task_delta(root: Path, runtime: dict[str, Any]) -> bool:
    if _CORE is None or not runtime:
        return False
    try:
        return bool(_CORE._task_files(root, runtime))
    except (AttributeError, OSError, ValueError):
        return False

def _infer_phase(root: Path, runtime: dict[str, Any]) -> str:
    ...
    if _blocking_findings(runtime):
        return 'correction'
    if _has_task_delta(root, runtime):
...
def _select_preset(*, strategy: str, phase: str, strict: bool, review_mode: str, failed_attempts: int, root_cause_known: bool | None, allow_xhigh: bool) -> tuple[str | None, list[str]]:
    ...
    if phase == 'review':
        ...
        return ('terra_high', ['incremental validated review is narrow but still benefits from high checking effort'])
    if phase == 'planning':
        if strict:
            return ('sol_high', ['sensitive planning has multiple constraints and expensive failure modes'])
        if strategy == 'quality':
...
def _adapter_hints(model: str, reasoning_effort: str) -> dict[str, Any]:
    return {'codex': {'model': model, 'reasoning_effort': reasoning_effort, 'interactive_action': 'use /model only at a session or major-phase boundary'}, 'opencode': {'model': f'openai/{model}', 'reasoning_effort': reasoning_effort, 'interactive_action': 'use /models or the model/variant selector only at a session or major-phase boundary'}}

def model_policy(root: Path, *, phase: str | None=None, strategy: str | None=None, review_mode: str | None=None, failed_attempts: int | None=None, root_cause_known: bool | None=None) -> dict[str, Any]:
    ...
    resolved_phase = str(phase or _infer_phase(root, runtime)).strip().lower()
    ...
    inferred_attempts = max(int(metrics.get('review_cycles', 0)), int(metrics.get('reopens', 0)))
    attempts = max(0, int(inferred_attempts if failed_attempts is None else failed_attempts))
    strict, signals = _risk_signals(runtime)
    preset_name, reasons = _select_preset(strategy=resolved_strategy, phase=resolved_phase, strict=strict, review_mode=resolved_review_mode, failed_attempts=attempts, root_cause_known=root_cause_known, allow_xhigh=bool(config['allow_xhigh']))
    if preset_name is None:
        return {'strategy': resolved_strategy, 'phase': resolved_phase, 'automatic': False, 'recommendation': None, 'reviewer_agent': None, 'signals': signals, 'reasons': reasons, 'warnings': warnings}
    ...
    recommendation = {**preset, 'preset': preset_name, 'switch_recommended': switch_recommended, 'apply_at': 'new session or major-phase boundary' if switch_recommended else 'keep current writer; apply to specialized subagent when applicable', 'adapters': _adapter_hints(preset['model'], preset['reasoning_effort'])}
    return {'strategy': resolved_strategy, 'phase': resolved_phase, 'automatic': True, 'strict': strict, 'review_mode': resolved_review_mode, 'failed_attempts': attempts, 'root_cause_known': root_cause_known, 'recommendation': recommendation, 'reviewer_agent': _reviewer_agent(preset_name, strict=strict, review_mode=resolved_review_mode), 'signals': signals, 'reasons': reasons, 'warnings': warnings, 'quality_floor': {'writer': 'Terra medium for ordinary implementation; Sol medium or higher for strict implementation', 'review': 'Terra high for narrow incremental review; Sol high for full or sensitive review', 'xhigh': 'never automatic without an explicitly unknown root cause and two bounded failed attempts'}}
