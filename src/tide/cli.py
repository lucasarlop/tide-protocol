...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    out(
        args,
        {"project": str(root), "locks": str(directory.relative_to(root))},
        title="Projeto inicializado",
    )
...
def cmd_prepare(args: argparse.Namespace) -> None:
    out(
        args,
        prepare(project_root(), args.task, args.file or []),
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    out(args, result, title="Validação")
...
def cmd_authorize(args: argparse.Namespace) -> None:
    out(
        args,
        authorize(project_root(), args.gate or [], all_gates=args.all),
        title="Hardgates atualizados",
    )


def cmd_review_packet(args: argparse.Namespace) -> None:
    out(args, review_packet(project_root()), title="Pacote de review")
...
def cmd_review(args: argparse.Namespace) -> None:
    ...
    out(
        args,
        record_review(project_root(), approved=args.approved, findings=findings),
...
def cmd_lock_draft(args: argparse.Namespace) -> None:
    ...
    out(
        args,
        {
            ...
            "status": (
                "draft; analyze live code, fill invariants and validations, "
                "then run tide lock validate"
            ),
        },
...
def cmd_setup(args: argparse.Namespace) -> None:
    ...
    out(args, result, title="Configuração global")
...
def cmd_uninstall(args: argparse.Namespace) -> None:
    ...
    title = "Simulação de remoção" if args.dry_run else "Tide removido"
    out(args, result, title=title)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="tide",
        description="Minimal quality protocol for AI coding agents",
    )
    ...
    update = sub.add_parser(
        "update",
        help="update the uv tool and refresh installed adapters",
    )
    ...
    uninstall = sub.add_parser(
        "uninstall",
        help="remove Tide integrations and the uv tool",
    )
    ...
    uninstall.add_argument(
        "--keep-tool",
        action="store_true",
        help="remove integrations but keep the uv tool",
    )
