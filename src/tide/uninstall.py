...
def uninstall_global(
    *,
    ...
    if codex:
        ...
        actions.extend(
            [
                f"remove bootstrap block -> {agents}",
                f"remove reviewer -> {reviewer}",
                f"remove Tide MCP block -> {config}",
            ]
        )
        if not dry_run:
...
def _remove_codex_entries(path: Path) -> None:
    ...
    if TOML_START in current and TOML_END in current:
        ...
        if graph_marker in managed:
            preserved_graph = (
                graph_marker + managed.split(graph_marker, 1)[1].strip() + "\n"
            )
