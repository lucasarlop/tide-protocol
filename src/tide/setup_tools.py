...
def setup_global(*, codex: bool, opencode: bool, dry_run: bool = False) -> list[str]:
    ...
    if codex:
        ...
        if not dry_run:
            _patch_markdown(
                codex_dir / "AGENTS.md",
                asset_root.joinpath("adapters/codex/AGENTS.md").read_text(),
            )
            _copy_file(
                asset_root.joinpath("adapters/codex/tide-reviewer.toml"),
                codex_dir / "agents" / "tide-reviewer.toml",
            )
    ...
    if opencode:
        ...
        if not dry_run:
            _patch_markdown(
                bootstrap_path,
                asset_root.joinpath("adapters/opencode/AGENTS.md").read_text(),
            )
            _copy_file(
                asset_root.joinpath("adapters/opencode/tide-reviewer.md"),
                config_dir / "agents" / "tide-reviewer.md",
            )
            _merge_opencode(
                config_path,
...
def _patch_codex_config(path: Path, *, graph_command: str | None) -> None:
    ...
    lines = [
        ...
        "",
        "[mcp_servers.tide.tools.validate]",
        'approval_mode = "prompt"',
    ]
    if graph_command and not graph_already_configured:
        lines.extend(
            [
                ...
                f"command = {json.dumps(graph_command)}",
                'args = ["serve"]',
            ]
        )
    ...
    *,
    ...
    if path.exists():
        ...
        try:
            ...
            raise TideError(
                f"cannot safely update OpenCode config {path}: {exc}. "
